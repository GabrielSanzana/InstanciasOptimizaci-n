...
def simulated_annealing_cuda(T0, alpha, Tmin, max_iter, datos_estructura, capacidad_t, v_min_t, v_max_t, costo_alq_t, solucion_inicial, max_items_ini, max_items_final, device='cuda'):
    ...
    for iteracion in range(max_iter):
        ...
        if (iteracion + 1) % 100 == 0 or iteracion == 0:
            print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}]     SA Iteration {iteracion+1}/{max_iter}: Temp={T:.2f}, Current Obj={obj_actual:.2f}, Best Obj={mejor_obj:.2f}")
...
if __name__ == '__main__':
    ...
    FILE_PATH = 'rl1323_n13220_bounded-strongly-corr_05.ttp'
    ...
    alpha = 0.999
    Tmin = 0.45
    max_iters = 200 # Iteraciones para cada mini-SA
    ...
    K = 20000  # Number of generations (mini-SAs)
    ...
    for gen in range(K):
        # Each generation: apply simulated annealing from the current solution
        ...
        # Update current solution with the new one regardless of improvement to keep the cycle going
        current_solution = (new_tour, new_mask_gpu)

        # Save if improved overall
        if new_obj > best_obj:
            best_obj = new_obj
